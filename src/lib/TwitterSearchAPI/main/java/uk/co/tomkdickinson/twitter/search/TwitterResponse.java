...
public class TwitterResponse {
	...
	public TwitterResponse(boolean has_more_items, String items_html, String min_position,
			String refresh_cursor, long focused_refresh_interval) {
	...
	/*
	 * public long getNew_latent_count() { return new_latent_count; }
	 * 
	...
	public List<Tweet> getTweets() {
		...
		for (Element el : doc.getElementsByClass("js-stream-item")) {
			...
			try {
				favourites = Integer.parseInt(el
			...
			Elements photos = el.select("div.js-adaptive-photo");

			for (Element photo : photos) {
				imageURLs.add(photo.attr("data-image-url"));
			}

			Tweet tweet = new Tweet(id, text, userId, userName, userScreenName, createdAt, retweets,
					favourites, imageURLs);
			if (tweet.getId() != null) {
				tweets.add(tweet);
			}
		}
